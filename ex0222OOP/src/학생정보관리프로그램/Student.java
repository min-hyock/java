...
public class Student {
	...
	// 생성자의 특징
	// 1. 리턴타입을 지정조차 하지 않는다.(void도 작성 X)
	// 2. 생성자의 이름은 클래스 이름과 동일해야한다.(대소문자까지)
	// 3. 생성자도 결국 메소드이다.
	// 4. 매개변수가 아무것도 없는 생성자를 기본 생성자(default constructor)라고
	//	  부르는데 생략 가능하다.
	// 	  단, 새로운 나만의 생성자를 만들게 되면 기본생성자는 덮어씌워진다.(사라진다)
	// 5. 생성자도 오버로딩(중복정의) 가능하다.
	// 	  오버로딩이란 ?
	// -> 메소드의 이름과 리턴타입이 동잉한 상태에서 매개변수의 개수와 타입을 다르게
	// 	  중복으로 정의하는 메소드 정의 기법
	public Student() {
